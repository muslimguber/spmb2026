...
export const Quis1: React.FC<Quis1Props> = ({ theme, username, userClass, onComplete }) => {
  ...
  // Persistence keys
  const KEY_INDEX = `ipa_quis1_currentIndex`;
  const KEY_ANSWERS = `ipa_quis1_answers`;
  const KEY_SHUFFLED = `ipa_quis1_shuffledQuestions`;
  const KEY_SHOW_RESULT = `ipa_quis1_showResult`;
  ...
  if (shuffledQuestions.length === 0) {
    return (
      <div className="w-full min-h-[400px] flex flex-col items-center justify-center p-8 text-center space-y-4">
        ...
        <p className="text-lg font-black text-slate-800">Menyiapkan Soal Quis 1...</p>
      </div>
    );
  }
  ...
  const getMotivation = () => {
    if (percentage === 100) return { title: "Sempurna!", color: "text-emerald-500", message: "Luar biasa! Kamu memahami materi Modul 1 dengan sangat baik." };
      ...
      <div className="w-full max-w-2xl mx-auto py-6">
        ...
        >
          <div className="p-8 text-center text-white space-y-2" style={{ backgroundColor: theme.bgSidebar }}>
            ...
            <h2 className="text-3xl font-black">Hasil Quis 1</h2>
          ...
          <div className="p-10 text-center space-y-8">
            ...
            <div className="grid grid-cols-1 gap-4 max-w-xs mx-auto">
              ...
              <ThemeButton 
                ...
                onClick={() => {
                  window.open('https://s.shopee.co.id/70GyYt2MVB', '_blank');
    ...
    <div className="w-full max-w-2xl mx-auto space-y-6">
      <div className="bg-white/80 backdrop-blur-md p-4 rounded-2xl shadow-lg border-2 border-white/60 flex justify-between items-center px-6">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <h2 className="font-black text-slate-800 truncate whitespace-nowrap text-sm md:text-base">Quis 1: {config.title}</h2>
        </div>
