...
const App = () => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    const finalUsername = username.trim() || 'PENGUNJUNG';
    const finalUserClass = userClass.trim() || 'UMUM';

    const lastUser = localStorage.getItem('ipa_user');
    const currentUsername = finalUsername;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="pt-6 pb-2.5 px-2.5 border-b border-white/10">
          <div className="flex items-center gap-2 mb-2">
            ...
            <div>
              <h2 className="font-black text-[13px] tracking-tight leading-none uppercase">SPMB 2026-2027</h2>
              <p className="text-[10px] opacity-80 font-bold uppercase tracking-tight mt-1">SMPN 1 BENGKALIS</p>
            </div>
            <button 
              onClick={() => setSidebarOpen(false)}
        ...
        <div className="border-t border-white/10 px-3 py-4 pb-12 space-y-4">
          ...
          <button 
            ...
            className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 text-rose-400 font-bold uppercase tracking-widest text-[10px] transition-all"
          >
            <LogOut size={14} />
            <span>EXIT / KELUAR</span>
          </button>
        </div>
      </motion.aside>
      ...
      <main className="flex-1 h-screen overflow-y-auto relative flex flex-col">
        {/* Background decorations removed for flat look */}
